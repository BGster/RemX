...
| 条件 | 阈值 | 提醒 |
|------|------|------|
| `deprecated` 占比 | > 20% | "N 条过期记录，建议 pm gc" |
...
```bash
pm gc              # 软删除：标记 deprecated=1
pm gc --purge     # 物理删除：删除 deprecated 记录 + VACUUM
pm gc --dry-run   # 预览，不实际删除
```
...
`pm index <path>` 将三次写入（memories → chunks → memories_vec）包在同一个事务内：
